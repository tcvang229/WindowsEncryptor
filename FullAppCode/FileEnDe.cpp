...
void fileEncryption(char *fileName)
{
	...
	while (!inFile.eof())
	{
		...
		for (unsigned int i = 0; i < fileReader.length(); i++)
		{
...
void fileDecryption(char *fileName)
{
	std::string fileReader;
	std::ifstream inFile;
	std::ofstream outFile;
...
std::string outputFileContents(std::string fileName)
{	
